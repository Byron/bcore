...
class TestEnvironmentStack(TestComponentCoreBase):
    ...
    def test_context(self):
        ...
        assert(not cbcore.keys())
        bcore.environment.push(bottom_env_name)
        ctx = bcore.environment.context()
        assert(not cbcore.keys())
        bcore.environment.push(maya_env)
        ctx = bcore.environment.context()
        assert(cbcore.value(key_category, '') == category)
        assert(cbcore.value(key_name, '') == maya_env_name)
        assert(cbcore.value(key_version, 'X') == maya_version)
        bcore.environment.push("top")
        ctx = bcore.environment.context()
        assert(cbcore.value(key_category, '') == category)
        assert(cbcore.value(key_name, '') == maya_env_name)
        ...
        ctx = bcore.environment.context()
        assert(cbcore.value(key_category, '') == category)
        assert(cbcore.value(key_name, '') == houdini_env_name)
        assert(cbcore.value(key_version, 'X') == houdini_version)
        bcore.environment.pop()
        ctx = bcore.environment.context()
        assert(cbcore.value(key_category, '') == category)
        assert(cbcore.value(key_name, '') == houdini_env_name)
