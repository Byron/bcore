...
An implementation of semantic name handling is done in the *template framework* in *bcore.template*. 
...
- (*Semantic*)Nodes
 ...
 + Is called @ref bcore.template.base.ElementNode "ElementNode" in code
  
- (*Semantic*)Node Lists
 
 + They are similar to *sentences comprised of symbolic names*.
 + Can be transformed into a string with multiple symbols.
 + Is called @ref bcore.template.base.ElementNodeList "ElementNodeList" in code.
...
- *Rule Sets*
 ...
 + Is called @ref bcore.template.base.ElementNodeTree ElementNodeTree in code
...
- The information you would like to encode is contained in nested key-value dictionaries.
  ...
  Now you iterate the @ref bcore.template.base.ElementNodeTree "ElementNodeTree" to yield all 
  @ref bcore.template.base.ElementNodeList "ElementNodeList" instances which are able to fully embody 
  the information you provided. Each @ref bcore.template.base.ElementNode "ElementNode" in an 
  @ref bcore.template.base.ElementNodeList "ElementNodeList" contains meaning implied by the data 
  it may carry. You may further filter out those @ref bcore.template.base.ElementNodeList "ElementNodeLists" 
  whose @ref bcore.template.base.ElementNode "ElementNodes" don't match your criteria. 
    
  Finally you configure each @ref bcore.template.base.ElementNode "ElementNode" with the data you want 
...
- Provided that you know the @ref bcore.template.base.ElementNodeTree "ElementNodeTree" used to create 
  the *sentence*, iterate all @ref bcore.template.base.ElementNodeList "ElementNodeLists in the the tree 
  and try to infer the configuration of each @ref bcore.template.base.ElementNode "ElementNode from left 
  to right, and consume portions of the *sentence* from left to right. 
  @ref bcore.template.base.ElementNodeList "ElementNodeLists" that could consume the whole *sentence* 
  will be returned.
...
- Generally it is possible that a certain meaning is satisfied by multiple *sentences*, and that one *sentence* 
  ...
  @ref bcore.template.base.ElementNodeList "ElementNodeLists" to handle, no matter if you are encoding or 
  decoding *sentences*.
- If the code using this system does not assume a fixed grammar, it cannot assume any specific configuration 
  of @ref bcore.template.base.ElementNode "ElementNodes" in the obtained 
  @ref bcore.template.base.ElementNodeList "ElementNodeLists". All it may and must know about are the 
  properties of the @ref bcore.template.base.ElementNode "ElementNodes" to determine where to apply which 
  part of information.
