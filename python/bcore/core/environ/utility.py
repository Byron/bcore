#-*-coding:utf-8-*-
"""
@package tx.core.environ.utility
...
def file_environment(*paths, **kwargs):
    ...
    alter the current global context as defined in tx.environment to contain all environments obtained when
    creating ConfigHierarchyEnvironment instances for all the given paths.
    @return returned value is the altered tx.environment instance, just for convenience
    @note this will temporarily change the tx.environment, which is a rather expensive operation both in terms
    of IO and CPU
    ...
    size = -1
    for index, env in enumerate(tx.environment.stack()):
    ...
    try:
        while len(tx.environment) > size:
            popped_environments.append(tx.environment.pop())
        # end pop environments
        for path in paths:
            env = tx.environment.push(ConfigHierarchyEnvironment(path))
        ...
        if len(tx.environment) > size:
            tx.environment.pop(until_size = size)
